/*********************************************************************
 *
 * File: linux/kernel/drivers/stm/coredisplay/stih205/stih205.c
...
#include "soc/stih205/stih205device.h"
    ...
    #define HAS_DSUB 0
#endif
//Fix: missing in stxh205.h, use 7111 value
#define SYS_CFG 2

static const unsigned long whitelist[] = {
    STih205_REGISTER_BASE + STih205_DENC_BASE,
    STih205_REGISTER_BASE + STih205_DENC_BASE+PAGE_SIZE,
    STih205_REGISTER_BASE + STih205_DENC_BASE+(PAGE_SIZE*2),
    STih205_REGISTER_BASE + STih205_HDMI_BASE,
    _ALIGN_DOWN(STih205_REGISTER_BASE + STih205_BLITTER_BASE, PAGE_SIZE),
};

/* BDisp IRQs on h205: all aq sharing 0x1580, cq1 ???, cq2 ??? !! X111 values */
...
#warning Architecture not supported
#endif
		.main_output_id           = STih205_OUTPUT_IDX_VDP0_MAIN,
 ...
 && defined(__TDT__)
		.dvo_output_id            = -1,
#else
		.dvo_output_id            = STih205_OUTPUT_IDX_DVO0,
#endif
		.blitter_id               = STih205_BLITTER_IDX_VDP0_MAIN,
		.blitter_id_kernel        = STih205_BLITTER_IDX_KERNEL,
		...
		.mmio                     = _ALIGN_DOWN (STih205_REGISTER_BASE + STih205_BLITTER_BASE, PAGE_SIZE),
		.mmio_len                 = PAGE_SIZE,
	},
	{
		.owner                    = THIS_MODULE,
		...
		.main_output_id           = STih205_OUTPUT_IDX_VDP1_MAIN,
		.hdmi_output_id           = -1,
		.dvo_output_id            = -1,

		.blitter_id               = STih205_BLITTER_IDX_VDP1_MAIN,
		.blitter_id_kernel        = STih205_BLITTER_IDX_KERNEL,
		.blitter_type             = STMCORE_BLITTER_BDISPII,
...
#define GPIO_PIN_VSYNC stm_gpio(3,1) //TODO: get correct value
#define GPIO_PIN_HSYNC stm_gpio(3,2) //TODO: get correct value
...
{
	if (boot_cpu_data.type == CPU_STXH205)
	{
		int i;
...
//					sysconf_write(syscfg2_27, 1);
		...
		/*
		 * Enable DVO->DVP loopback. The h205 does not have external pads for
...
//				clk_enable(coredisplay_clks[i].clk);
		...
		printk(KERN_INFO "stmcore-display: STiH205 display probed\n");
		return 0;
	}
	printk(KERN_WARNING "stmcore-display: STiH205: platform unknown\n");
