/***********************************************************************
 *
 * File: soc/stih205/stih205hdmi.cpp
...
CSTih205HDMI::CSTih205HDMI(
    ...
    stm_hdf_hdmi_hardware_version_t hdfRevision): CSTmHDFormatterHDMI(pDev,
                                                    ...
                                                    STih205_HDMI_PCMPLAYER_BASE,
                                                    STih205_HDMI_SPDIF_BASE,
                                                    pMainOutput,
                                                    pVTG)
{
  ULONG val;
  ...
  m_ulSysCfgOffset = STih205_SYSCFG_BASE;
  m_ulAudioOffset  = STih205_AUDCFG_BASE;
  m_ulTVOutOffset  = STih205_MAIN_TVOUT_BASE;
  m_ulClkOffset    = STih205_CLKGEN_BASE;
...
bool CSTih205HDMI::Create(void)
{
   ...
   * base class Create().
   */
  m_pIFrameManager = new CSTmDMAIFrames(m_pDisplayDevice,m_ulHDMIOffset,STih205_REGISTER_BASE);
...
void CSTih205HDMI::StartAudioClocks(void)
{
  DENTRY();
  /*
   ...
   * does not work. Note we do the pcm and spdif clocks on h205; it could
  ...
  {
    val &= ~(AUD_FSYN_CFG_FS0_EN | AUD_FSYN_CFG_FS1_EN | AUD_FSYN_CFG_FS2_EN);
...
void CSTih205HDMI::SetupRejectionPLL(const stm_mode_line_t *mode,
                                     ULONG                  tvStandard)
{
  ULONG val;
...
bool CSTih205HDMI::PreConfiguration(const stm_mode_line_t *mode,
                                    ULONG                  tvStandard)
{
  ULONG val;
...
int CSTih205HDMI::GetAudioFrequency(void)
{
  ULONG sdiv;
...
bool CSTih205HDMI::SetInputSource(ULONG src)
{
  DENTRY();

  /*
   * h205 only has 2 channel PCM going to the HDMI
   */
