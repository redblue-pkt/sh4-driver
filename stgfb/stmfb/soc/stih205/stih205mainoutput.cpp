/***********************************************************************
 *
 * File: soc/stih205/stih205mainoutput.cpp
...
//
CSTih205MainOutput::CSTih205MainOutput(
    CDisplayDevice     *pDev,
    CSTmSDVTG          *pVTG1,
    CSTmSDVTG          *pVTG2,
    CSTmTVOutDENC      *pDENC,
    ...
    stm_plane_id_t      sharedGDPPlaneID): CSTmHDFormatterOutput(pDev, STih205_MAIN_TVOUT_BASE, STih205_AUX_TVOUT_BASE, STih205_HD_FORMATTER_BASE, pVTG1, pVTG2, pDENC, pMixer, pFSynth, pAWG)
{
...
void CSTih205MainOutput::StartSDInterlacedClocks(const stm_mode_line_t *mode)
{
#if defined(__TDT__) && defined(CONFIG_CPU_SUBTYPE_STXH205)
...
void CSTih205MainOutput::StartSDProgressiveClocks(const stm_mode_line_t *mode)
{
  ULONG val;
...
void CSTih205MainOutput::StartHDClocks(const stm_mode_line_t *mode)
{
  ULONG val = 0;
...
void CSTih205MainOutput::SetMainClockToHDFormatter(void)
{
  DENTRY();
...
bool CSTih205MainOutput::ShowPlane(stm_plane_id_t planeID)
{
  DEBUGF2(2,("CSTih205MainOutput::ShowPlane %d\n",(int)planeID));
  ...
  if(planeID == m_sharedGDPPlaneID)
  {
       ...
       m_pCurrentMode->TimingParams.ulPixelClock == 135135000)
    {
      ...
      WriteClkReg(CKGB_CLK_SRC, val);
      DEBUGF2(2,("CSTih205MainOutput::ShowPlane GDP3 clock set to FS1\n"));
    }
    else
    {
      /*
      ...
      ULONG val = ReadClkReg(CKGB_CLK_SRC) & ~CKGB_SRC_GDP3_FS1_NOT_FS0;
      WriteClkReg(CKGB_CLK_SRC, val);
      DEBUGF2(2,("CSTih205MainOutput::ShowPlane GDP3 clock set to FS0\n"));
    }
  }
  else if (planeID == m_sharedVideoPlaneID)
  {
    ...
    DEBUGF2(2,("CSTih205MainOutput::ShowPlane VID2 clock set to HD pixel clock\n"));
  }
...
void CSTih205MainOutput::EnableDACs(void)
{
  ULONG val;
...
void CSTih205MainOutput::DisableDACs(void)
{
  DENTRY();
  ULONG val;
