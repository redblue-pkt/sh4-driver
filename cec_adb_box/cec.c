...
//3f821100 - switching to hdmi1
//3f822100 - switching to hdmi2

//3f821000 - switching to hdmi1
//3f822000 - switching to hdmi2


int proc_cec_write(struct file *file, const char __user *buf, unsigned long count, void *data)
{
	char    *page;
	char    *myString;
	ssize_t ret = -ENOMEM;
...
int proc_cec_read(char *page, char **start, off_t off, int count, int *eof, void *data_unused)
{
	printk("%s\n", __func__);
	return 0;
}
...
static int __init init_cec_module(void)
{
	...
	if (cec_tx == NULL)
	{
		debug_cec("FAIL: request pin 4 4\n");
		goto err;
	}
	stpio_set_pin (cec_tx, 1);
	...
	if (request_irq(120, asc_irq, IRQF_DISABLED, "asc_irq", NULL))
	{
		debug_cec("FAIL: request irq asc\n");
		goto err;
	}
...
//	t = BAUDRATE_VAL_M1(100000, f);	 // 10 bit x 10us = 100us
	t = BAUDRATE_VAL_M1(33333, f);	// 10 bit x 30us = 300us
	...
	if (cec_rx == NULL)
	{
		debug_cec("FAIL: request pin 3 7\n");
		goto err;
	}
	stpio_set_pin (cec_rx, 1);
#endif
#if 0
	if (request_irq(126, asc_irq, IRQF_DISABLED, "asc_irq", NULL))
	{
		debug_cec("FAIL : request irq asc\n");
		goto err;
	}
