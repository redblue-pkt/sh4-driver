...
#define _CONFIG_SH_EXTERNAL_CLOCK 27000000  // 27MHz
#endif
...
#define _CONFIG_SH_EXTERNAL_CLOCK 30000000  // 30 MHz
#endif
...
static int read_ratio(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
 ...
 || defined(STXH205)
	...
	switch (value)
	{
		case 0:
		{
			val_sysaclkout = freq_pll0 / sh4_div;
			break;
		}
		case 1:
		{
			val_sysaclkout = freq_pll0 / sh4_ic_div;
			break;
		}
		case 2:
		{
			val_sysaclkout = freq_pll0 / module_div;
			break;
		}
		case 3:
		{
		...
		case 9:
		{
			val_sysaclkout = freq_pll1 / 2;
			break;
		}
		case 10:
		case 11:
		{
			val_sysaclkout = freq_pll1 / 4;
			break;
		}
	}
...
static int write_pll1_ndiv_mdiv(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct clk *clk;
...
static int write_pll0_ndiv_mdiv(struct file *file, const char *buffer, unsigned long count, void *data)
{
#if defined(STB7100)
	struct clk *clk;
	unsigned long sta;
...
//  unsigned long regdata1;
 ...
 || defined(STXH205)
	...
	if (mdiv < 0 || mdiv > 255)
	{
		dprintk("[CPU_FREQU] mdiv not correct, use another PLL0 frequency (%d)\n", mdiv);
		return count;
	}
	if (ndiv < 3 || ndiv > 255)
	{
		dprintk("[CPU_FREQU] ndiv not correct, use another PLL0 frequency (%d)\n", ndiv);
		return count;
	}
#if defined(STB7100)
	ctrl_outl(0xC0DE, CKGA_LCK);
...
//  }
 ...
 || defined(STXH205)
	...
	if (clk)
	{
		...
		// measured bogomips not always correct in running system
...
#if defined(STB7100)
static int write_sh4_ratio(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long value;
...
static int write_sh4_ic_ratio(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long value;
...
static int write_module_ratio(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long value;
...
static int write_slim_ratio(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long value;
...
static int write_pll1_fdma_bypass(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned long value;
...
static int write_sysaclkout(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
static int write_pll1_clk_div(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
static int write_m_hz(struct file *file, const char *buffer, unsigned long count, void *data)
{
