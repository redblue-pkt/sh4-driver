...
static void VideoSetEvent(struct DeviceContext_s *Context, struct stream_event_s *Event);
#ifdef __TDT__
...
static int VideoIoctlSetEncoding(struct DeviceContext_s *Context, unsigned int Encoding)
{
	...
	switch (Context->VideoState.play_state)
	{
		...
		default:
		{
			...
			sigfillset(&Newsigs);
			sigprocmask(SIG_BLOCK, &Newsigs, &Oldsigs);
			Result = DvbStreamSwitch(Context->VideoStream, BACKEND_PES_ID, VideoContent[Context->VideoEncoding]);
			sigprocmask(SIG_SETMASK, &Oldsigs, NULL);
			/*
			if (Result == 0)
			 Result = VideoIoctlSetId (Context, Context->VideoId);
...
/*}}}*/
/*{{{ VideoIoctl*/
static int VideoIoctl(struct inode *Inode, struct file *File, unsigned int IoctlCode, void *Parameter)
{
	struct dvb_device *DvbDevice = (struct dvb_device *)File->private_data;
	struct DeviceContext_s *Context = (struct DeviceContext_s *)DvbDevice->priv;
	struct DvbContext_s *DvbContext = Context->DvbContext;
	...
	*/
	...
	switch (IoctlCode)
	{
		...
		case VIDEO_SELECT_SOURCE:
			Result = VideoIoctlSelectSource(Context, (video_stream_source_t)Parameter);
			break;
		case VIDEO_SET_BLANK:
			Result = VideoIoctlSetBlank(Context, (unsigned int)Parameter);
			break;
		case VIDEO_GET_STATUS:
			Result = VideoIoctlGetStatus(Context, (struct video_status *)Parameter);
			break;
		case VIDEO_GET_EVENT:
			Result = VideoIoctlGetEvent(Context, (struct video_event *)Parameter, File->f_flags);
			break;
		case VIDEO_GET_SIZE:
			Result = VideoIoctlGetSize(Context, (video_size_t *)Parameter);
			break;
		case VIDEO_SET_DISPLAY_FORMAT:
			Result = VideoIoctlSetDisplayFormat(Context, (video_displayformat_t)Parameter);
			break;
		case VIDEO_STILLPICTURE:
			Result = VideoIoctlStillPicture(Context, (struct video_still_picture *)Parameter);
			break;
		...
		case VIDEO_GET_CAPABILITIES:
			...
			break;
		case VIDEO_GET_PTS:
			Result = VideoIoctlGetPts(Context, (unsigned long long int *)Parameter);
			break;
		case VIDEO_GET_FRAME_COUNT:
			Result = VideoIoctlGetFrameCount(Context, (unsigned long long int *)Parameter);
			break;
		case VIDEO_SET_ID:
		...
		case VIDEO_SET_STREAMTYPE:
			Result = VideoIoctlSetStreamType(Context, (unsigned int)Parameter);
			break;
		case VIDEO_SET_FORMAT:
			Result = VideoIoctlSetFormat(Context, (unsigned int)Parameter);
			break;
		case VIDEO_SET_SYSTEM:
			Result = VideoIoctlSetSystem(Context, (video_system_t)Parameter);
			break;
		case VIDEO_SET_HIGHLIGHT:
			Result = VideoIoctlSetHighlight(Context, (video_highlight_t *)Parameter);
			break;
		case VIDEO_SET_SPU:
			Result = VideoIoctlSetSpu(Context, (video_spu_t *)Parameter);
			break;
		case VIDEO_SET_SPU_PALETTE:
			Result = VideoIoctlSetSpuPalette(Context, (video_spu_palette_t *)Parameter);
			break;
		case VIDEO_GET_NAVI:
			Result = VideoIoctlGetNavi(Context, (video_navi_pack_t *)Parameter);
			break;
		case VIDEO_SET_ATTRIBUTES:
			Result = VideoIoctlSetAttributes(Context, (video_attributes_t *)Parameter);
			break;
		case VIDEO_GET_FRAME_RATE:
		...
		case VIDEO_SET_SPEED:
			...
			break;
		case VIDEO_DISCONTINUITY:
			Result = VideoIoctlDiscontinuity(Context, (video_discontinuity_t)Parameter);
			break;
		case VIDEO_STEP:
			Result = VideoIoctlStep(Context);
			break;
		case VIDEO_SET_PLAY_INTERVAL:
			Result = VideoIoctlSetPlayInterval(Context, (video_play_interval_t *)Parameter);
			break;
		case VIDEO_SET_SYNC_GROUP:
			Result = VideoIoctlSetSyncGroup(Context, (unsigned int)Parameter);
			break;
		case VIDEO_COMMAND:
			Result = VideoIoctlCommand(Context, (struct video_command *)Parameter);
			break;
		case VIDEO_GET_PLAY_TIME:
			Result = VideoIoctlGetPlayTime(Context, (video_play_time_t *)Parameter);
			break;
		case VIDEO_GET_PLAY_INFO:
			Result = VideoIoctlGetPlayInfo(Context, (video_play_info_t *)Parameter);
			break;
		case VIDEO_SET_CLOCK_DATA_POINT:
			Result = VideoIoctlSetClockDataPoint(Context, (video_clock_data_point_t *)Parameter);
			break;
		case VIDEO_SET_TIME_MAPPING:
			Result = VideoIoctlSetTimeMapping(Context, (video_time_mapping_t *)Parameter);
			break;
		default:
...
static ssize_t VideoWrite(struct file *File, const char __user *Buffer, size_t Count, loff_t *ppos)
{
	...
	if (!DvbStreamBufferFree(Context->VideoStream) && ((File->f_flags & O_NONBLOCK) != 0))
	{
		mutex_unlock(Context->ActiveVideoWriteLock);
		return -EWOULDBLOCK;
	}
...
static unsigned int VideoPoll(struct file *File, poll_table *Wait)
{
	...
	if (((File->f_flags & O_ACCMODE) != O_RDONLY) && (Context->VideoStream != NULL))
	{
		...
		{
			if (DvbStreamBufferFree(Context->VideoStream))
