		      ...
		      void *Parameter)
{
	...
	switch (IoctlCode)
	{
		...
		case AUDIO_SET_ENCODING:
			...
			break;
		case AUDIO_FLUSH:
			Result = AudioIoctlFlush(Context, (unsigned int)Parameter);
			break;
		...
		case AUDIO_SET_PLAY_INTERVAL:
			Result = AudioIoctlSetPlayInterval(Context, (audio_play_interval_t *)Parameter);
			break;
		...
		case AUDIO_SET_CLOCK_DATA_POINT:
			Result = AudioIoctlSetClockDataPoint(Context, (audio_clock_data_point_t *)Parameter);
			break;
		case AUDIO_SET_TIME_MAPPING:
			Result = AudioIoctlSetTimeMapping(Context, (audio_time_mapping_t *)Parameter);
			break;
		default:
...
static ssize_t AudioWrite(struct file *File, const char __user *Buffer, size_t Count, loff_t *ppos)
{
	...
	if (!DvbStreamBufferFree(Context->AudioStream) && ((File->f_flags & O_NONBLOCK) != 0))
	{
		mutex_unlock(&(Context->AudioWriteLock));
		return -EWOULDBLOCK;
	}
