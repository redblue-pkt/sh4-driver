...
#define AssertNonZeroReferenceCount( s ) do { if( ReferenceCount == 0 ) {report( severity_error, "%s - Attempt to act on a buffer that has a reference count of zero.\n", s ); /*dagobert try this->Dump(); Manager->Dump();*/}/*return BufferError;*/ } while(0)
#else
...
#else
#if defined(QBOXHD) || defined(QBOXHD_MINI)
#define AssertNonZeroReferenceCount( s ) do { if( ReferenceCount == 0 ) {report( severity_error, "%s - Attempt to act on a buffer that has a reference count of zero.\n", s ); this->Dump(); Manager->Dump();}/*return BufferError;*/ } while(0)
#else
				...
				Datum = Datum->Next)
		{
			...
			switch (Datum->Descriptor->AllocationSource)
			{
				...
				case NoAllocation:
					...
					break;
				default:
					break; // No Action, already returned error
			}
