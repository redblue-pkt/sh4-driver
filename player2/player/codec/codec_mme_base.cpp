...
CodecStatus_t Codec_MmeBase_c::Reset(void)
{
	...
	memset(BufferState, 0x00, MAX_DECODE_BUFFERS * sizeof(CodecBufferState_t));
...
//
	...
	memset(DecodeTimes, 0x00, 16 * MAX_DECODE_BUFFERS * sizeof(unsigned long long));
...
CodecStatus_t Codec_MmeBase_c::GetDecodeBuffer(void)
{
	...
	if (Status != BufferNoError)
	{
		report(severity_error, "Codec_MmeBase_c::GetDecodeBuffer(%s) - Failed to obtain a decode buffer from the decode buffer manager.\n", Configuration.CodecName);
		return Status;
	}
