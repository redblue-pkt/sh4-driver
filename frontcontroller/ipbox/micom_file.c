...
int micomSetIcon(int which, int on)
{
 ...
 && !defined(CUBEREVO_MINI_FTA)
	...
	{
		printk("[micom] icon number out of range %d\n", which);
		return -EINVAL;
	}
  ...
  #endif
	dprintk(100, "%s <\n", __func__);
#endif  // not 250HD and not mini fta
	return res;
}
 ...
 *
 * expected time format: YYMMDDhhmm
 */
...
int micomSetTime(char *time)
{
	...
	dprintk(10, "date: %c %c %c %c %c %c\n", time[0], time[1], time[2], time[3], time[4], time[5]);
	dprintk(10, "time: %c %c %c %c %c %c\n", time[6], time[7], time[8], time[9], time[10], time[11]);
...
int micomGetMicom(void)
{
 ...
 || defined(CUBEREVO_MINI_FTA)  /* fixme: not sure if true for MINI_FTA2 & CUBEREVO250HD!!! */
	micom_year  = 2008;
	micom_month = 4;
#else
	memset(buffer, 0, sizeof(buffer));
