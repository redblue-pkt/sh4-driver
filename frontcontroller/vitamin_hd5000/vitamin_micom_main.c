/*
 * vitamin_micom_main.c  for Showbox Vitamin HD 5000
 *
 ...
 * Description:
 *
 * Showbox Vitamin HD 5000 MICOM Kernelmodule ported from MARUSYS uboot source,
 ...
 * Date     By              Description
 * --------------------------------------------------------------------------------------
 * 20190522 Audioniek       procfs added.
 *
 ****************************************************************************************/
...
int getLen(int expectedLen)
{
	...
	while (1)
	{
		if (RCVBuffer[j] == 0x0d)  // 0x0d is end of response?
		{
...
{
	dprintk(30, "BufferStart %d, BufferEnd %d, len %d\n", RCVBufferStart, RCVBufferEnd, getLen(-1));
...
static void processResponse(void)
{
	...
	if (expectEventData)
	{
		dprintk(10, "%s: Response byte = 0x%02x\n", __func__, expectEventData);
		switch (expectEventData)
		{
			...
			{
				dprintk(0, "Invalid Response %02x\n", expectEventData);
...
static irqreturn_t FP_interrupt(int irq, void *dev_id)
{
	...
	while (*ASC_X_INT_STA & ASC_INT_STA_RBF)
	{
		...
		if (RCVBufferStart == RCVBufferEnd)
		{
			dprintk(0, "FP: RCV buffer overflow!!!\n");
		}
	}
#if 0
	if (paramDebug >= 201)
...
int micomTask(void *dummy)
{
	...
	while (1)
	{
		...
		{
			dprintk(0, "wait_event_interruptible failed\n");
			continue;
		}
		if (RCVBufferStart != RCVBufferEnd)
		{
...
static int __init micom_init_module(void)
{
	...
	*ASC_X_CTRL = *ASC_X_CTRL | ASC_CTRL_FIFO_EN;
	...
	{
		dprintk(0, "unable to get major %d for VFD/MICOM\n", VFD_MAJOR);
	}
	create_proc_fp();
	dprintk(100, "%s <\n", __func__);
	return 0;
...
{
	dprintk(0, "Vitamin MICOM frontcontroller module unloading\n");
	remove_proc_fp();
