...
int YWPANEL_FP_ParseI2cData(YWPANEL_FPData_t  *data,YWPANEL_I2CData_t	 *I2CData)
{
	...
	switch(dataType)
	{
		...
		case YWPANEL_INIT_INSTR_GETVERSION: /*get version */
		{
			...
			{
				ywtrace_print(TRACE_ERROR,"%s::unexpected dataType: %0x @%d\n",__FUNCTION__,data->dataType,__LINE__);
			}
...
int  YWPANEL_FP_GetVersion(YWPANEL_Version_t *version)
{
	...
	/* Deaktiviert das GetVersions ergebniss damit der SpiderBox HL101 den treiber weiter laden kann, brauchen diese daten nicht*/
	/*if(YWPANEL_FP_SendData(&data) != true)
	{
		ywtrace_print(TRACE_ERROR,"YWPANEL_FP_SendData failed [%d]\n",__LINE__);
		return false;
...
static u8 ywpanel_led_map[0x80] =
{
	...
	0xce, 0xe6, 0x0a, 0xb6, 0x1e, 0x0e, 0x46, 0x56, 0x6e, 0x76, 0xda, 0x9c, 0x00, 0xf0, 0x00, 0x10,
...
int YWPANEL_VFD_Init(void)
{
	...
	/*		return ErrorCode; */
	/* may be a SpiderBox HL101 - should we use YWVFD_STAND_BY or YWVFD_COMMON as display type? */
