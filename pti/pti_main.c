...
static void PtiWrite(volatile unsigned short int *addr,unsigned short int value)
{
	...
	if (((unsigned)addr & 2))
	{
		old_value = (old_value & 0x0000ffff) | value << 16;
	}
	else
	{
		old_value = (old_value & 0xffff0000) | value;
	}
	*addr2 = old_value;
}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,30)
		dma_cache_inv((void*)&internal->back_buffer[offset], count * PACKET_SIZE);
#else
		invalidate_ioremap_region(0, internal->back_buffer, offset, count * PACKET_SIZE);
#endif
		...
		if (demultiplex_dvb_packets != NULL)
		{
	...
	#endif
			{
				...
				if (count1[tag] >= AUX_COUNT)
				{
					...
					/* inject the packets */
	#if defined(SPARK7162)
					demultiplex_dvb_packets(internal->demux[internal->demux_tag[tag]], auxbuf[tag], count1[tag]);
	#else
					demultiplex_dvb_packets(internal->demux[tag], auxbuf[tag], count1[tag]);
	#endif
					pTo[tag] = auxbuf[tag];
					count1[tag] = 0;
				}
...
static void process_pti_dma(unsigned long data)
{
	...
	else
	{
		...
		if ((dma_0_buffer_rp & ~0xf) == (dma_0_buffer_top & ~0xf))
		{
			dma_0_buffer_rp = dma_0_buffer_base;
		}
		/* Calculate the amount of bytes used in the buffer */
		if (dma_0_buffer_rp <= pti_wp)
		{
			num_packets = (pti_wp - dma_0_buffer_rp) / PACKET_SIZE;
		}
		else
		{
...
static void process_pti_dma(unsigned long data)
{
	...
	else
	{
		...
		if (pti_rp <= pti_wp)
		{
			size_first = pti_wp - pti_rp;
		}
		else
		{
			size_first = pti_top - pti_rp;
		}
		...
		if (num_packets)
		{
			...
			if (!*internal->pread)
			{
				...
				if (new_rp > pti_top)
				{
					dprintk("You b@*tard you killed kenny\n");
				}
...
void pti_hal_init ( struct stpti *pti , struct dvb_demux* demux, void (*_demultiplex_dvb_packets)(struct dvb_demux* demux, const u8 *buf, int count))
{
 ...
 || defined(OPT9600PRIMA)
	...
	internal->vSlots = kmalloc(sizeof(struct tSlot *) * 32,	GFP_KERNEL );

	for (vLoopSlots = 0; vLoopSlots < 32; vLoopSlots++)
	{
		internal->vSlots[vLoopSlots] = kmalloc(sizeof(struct tSlot), GFP_KERNEL);
...
int pti_hal_slot_clear_pid ( int session_handle, int slot_handle )
{
	...
	printk("%s slot = %d, tc = %d, num = %d\n", __FUNCTION__, slot_handle,
		internal->vSlots[slot_handle]->tcIndex, *internal->num_pids);
...
int pti_hal_slot_link_buffer(int session_handle, int slot_handle, int bufType)
{
	return 0;
}
