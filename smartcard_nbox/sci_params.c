...
static int Set_Clock(SCI_CONTROL_BLOCK *sci)
{
...
static int Set_Clock_Val(SCI_CONTROL_BLOCK *sci,U32 div)
{
	PDEBUG(">>\n");
	dprintk(1, "Setting clock div:%d\n",div);
...
SCI_ERROR sci_set_para_ETU(SCI_CONTROL_BLOCK *sci, SCI_PARAMETERS *p_sci_parameters)
{
    ...
    if((p_sci_parameters->ETU >= SCI_MIN_ETU) && (p_sci_parameters->ETU <= SCI_MAX_ETU))
    {
		...
		if(sci->clk==357 || sci->clk==358)
		{
			...
			{
				case 4:
 					sci->clk=417;//Set_Clock(sci);
					Set_Clock_Val(sci,12);
				...
				break;
				case 5:
 					sci->clk=500;//Set_Clock(sci);
					Set_Clock_Val(sci,10);
