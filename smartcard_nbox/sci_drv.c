...
#include "sci_7100.h"
#elif defined(CONFIG_CPU_SUBTYPE_STX7111) || defined(UFS912) || defined(SPARK) || defined(ADB2850) || defined(HS7810A) || defined(DSI87)
#include "sci_7111.h"
...
static char reset0_off=1;

static char vcc0_on=1;
static char vcc0_off=0;

static char reset1_on=0;
static char reset1_off=1;

static char vcc1_on=1;
static char vcc1_off=0;


//dla tda8004 invert on vcc
//static char vcc1_on=0;
//static char vcc1_off=1;
...
void smartcard_reset(SCI_CONTROL_BLOCK *sci, unsigned char wait)
{
	...
	stpio_set_pin(sci->cmdvcc, vcc0_on);//stpio_set_pin(sci->cmdvcc, 1);
	mdelay(500);//stabilizacja zasialnia
	stpio_set_pin(sci->reset, reset0_on); //stpio_set_pin(sci->reset, 0); 
       }
    else if (sci->id == 1){
        disable_irq(SCI1_INT_RX_TX);
	stpio_set_pin(sci->cmdvcc, vcc1_on);//stpio_set_pin(sci->cmdvcc, 1);
	mdelay(500);//stabilizacja zasialnia
	stpio_set_pin(sci->reset, reset1_on); //stpio_set_pin(sci->reset, 0); 
        }
	else
	{
    ...
    /* Reset high */
    stpio_set_pin(sci->reset, reset0_off);//stpio_set_pin(sci->reset, 1);
    }
    else if (sci->id == 1)
    {
        /* Reset RX FIFO */
    ...
    /* Reset high */
    stpio_set_pin(sci->reset, reset1_off);//stpio_set_pin(sci->reset, 1);
    }
...
static void sci_detect_change(SCI_CONTROL_BLOCK *sci)
{
	...
	sci->atr_status = SCI_WITHOUT_ATR;
        /* VCC cmd low */
        if (sci->id == 0)
        stpio_set_pin(sci->cmdvcc,vcc0_off);//stpio_set_pin(sci->cmdvcc, 0);
        else
        stpio_set_pin(sci->cmdvcc,vcc1_off);//stpio_set_pin(sci->cmdvcc, 0);
...
#if defined(CONFIG_CPU_SUBTYPE_STB7100) || defined(CONFIG_CPU_SUBTYPE_STX7100) || defined(CONFIG_SH_ST_MB442) || defined(CONFIG_SH_ST_MB411) || defined(CONFIG_CPU_SUBTYPE_STX7111) || defined(HS7810A) || defined(UFS912) || defined(SPARK) || defined(ADB2850) || defined(DSI87)
...
#if defined(CONFIG_CPU_SUBTYPE_STX7111) || defined(UFS912) || defined(SPARK) || defined(ADB2850) || defined(HS7810A) || defined(DSI87)
                  ...
                  unsigned long ioctl_param  /* The parameter to it */)
{
    ...
    switch (ioctl_num)
    {
		...
		dprintk(1, "f is set to %d\n", sci_param.f);
            if ((sci_param.f/1000000)>0) sci_param.f=sci_param.f/1000000;
            if ((sci_param.f/100)>0) sci_param.f=sci_param.f/100;
	    	if (sci_param.f>6) sci_param.f=6;
		dprintk(1, "f is set to %d\n", sci_param.f);
...
static int __init sci_module_init(void)
{
    ...
    else {reset0_on=0;reset0_off=1;reset1_on=0;reset1_off=1;}
