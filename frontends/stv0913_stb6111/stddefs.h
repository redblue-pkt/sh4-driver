...
typedef unsigned short ST_DriverId_t;

/* This is a CPU identifier */
typedef unsigned char ST_CPU_t;
#define NULL_CPU ((ST_CPU_t) -1)

#define INCLUDE_GetRevision() ((ST_Revision_t) "INCLUDE-REL_2.1.37")
...
typedef struct
{
	U32   size;      /*! Size of the page */
	void* addr;      /*! Start address of the page may be physical, virtual,... */
} ST_ScatterPage_t ;

/** Scatter List definition.
    a ST_ScatterList_t* is like a void*: you don't have the size of the buffer,
    even if with a ST_ScatterList_t* you can have a 'max' size, but don't
    count on it always use a ST_ScatterList_t* exactly like a void*           */
typedef struct
{
	U32                        nbPages;  /*! numbers of pages */
	ST_ScatterPage_t           pages[];  /*! array of 'numbers' pages */
} ST_ScatterList_t;
