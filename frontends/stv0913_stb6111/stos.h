...
#else
#define STOS_InvalidateRegion(Addr, size)           invalidate_ioremap_region((U32)STOS_VirtToPhys(Addr), (void *)(Addr), 0, (size_t)(size))
#define STOS_FlushRegion(Addr, size)                writeback_ioremap_region((U32)STOS_VirtToPhys(Addr), (void *)(Addr), 0, (size_t)(size))
#define STOS_PurgeRegion(Addr, size)                flush_ioremap_region((U32)STOS_VirtToPhys(Addr), (void *)(Addr), 0, (size_t)(size))
#endif
...
STOS_Error_t STOS_InterruptEnableWakeup(unsigned int Number);
/* STOS interrupt map: get OS-specific interrupt number*/
...
/* Interrupt remapping */
#define STOS_InterruptMapGet(Number)      Number
...
#if defined(ST_OSLINUX) && defined(MODULE)
int  STOS_SemaphoreWaitInterruptible(STOS_Semaphore_t * Semaphore_p, BOOL Interruptible);
void *STOS_MessageQueueReceiveInterruptible(STOS_MessageQueue_t* MessageQueue, BOOL Interruptible);
void *STOS_MessageQueueReceiveTimeoutInterruptible(STOS_MessageQueue_t* MessageQueue, STOS_Clock_t * ticks, BOOL Interruptible);
#elif defined (ST_OS21)
#define STOS_SemaphoreWaitInterruptible(Semaphore_p,Interruptible)      STOS_SemaphoreWait((Semaphore_p));
#define STOS_MessageQueueReceiveInterruptible(MessageQueue,Interruptible)   STOS_MessageQueueReceive((MessageQueue));
#define STOS_MessageQueueReceiveTimeoutInterruptible(MessageQueue, ticks, Interruptible)    STOS_MessageQueueReceiveTimeout((MessageQueue), (ticks));
#endif /* defined(ST_OSLINUX) && defined(MODULE) */
...
#define STOS_TaskDelayUs(microsec)    do{                                      \
    U32     ClksPerSecs = STOS_GetClocksPerSecond();                             \
