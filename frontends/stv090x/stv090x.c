...
static struct stv090x_reg stxh205_initval[] =
{
	/* demod2 */
	{ STV090x_OUTCFG,         0x00 },
	{ STV090x_AGCRF1CFG,      0x11 },
...
/* Cut 2.0 Long Frame Tracking CR loop */
static struct stv090x_long_frame_crloop stxh205_s2_crl_cut20[] =
{
	/* MODCOD          2MPon 2MPoff 5MPon 5MPoff 10MPon 10MPoff 20MPon 20MPoff 30MPon 30MPoff */
	{STV090x_QPSK_12,  0x1c, 0x3c,  0x1b, 0x3c,  0x3a,  0x1c,   0x0b,  0x1c,   0x0b,  0x3b},
	{STV090x_QPSK_35,  0x2c, 0x3c,  0x2b, 0x2c,  0x3a,  0x0c,   0x0b,  0x1c,   0x0b,  0x3b},
...
static int stv090x_read_reg(struct stv090x_state *state, unsigned int reg)
{
	...
	if (ret != 2)
	{
		if (ret != -ERESTARTSYS)
		{
			dprintk(FE_ERROR, "Read error, Reg=[0x%02x], Status=%d\n", reg, ret);
		}
...
static int stv090x_i2c_gate_ctrl(struct dvb_frontend *fe, int enable)
{
	...
	{
		if ((state->device != STX7111) && (state->device != STXH205))
		{
...
err:
	dprintk(1, "stv090x_set_max_srate: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_set_min_srate: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_set_vit_thacq: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_set_vit_thtracq: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_set_viterbi: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_stop_modcod: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_activate_modcod: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_activate_modcod_single: I/O error\n");
	return -1;
}
...
err:
	mutex_unlock(&demod_lock);
	dprintk(1, "stv090x_vitclk_ctl: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_dvbs_track_crl: I/O error\n");
	return -1;
}
...
static int stv090x_delivery_search(struct stv090x_state *state)
{
	...
	switch (state->search_mode)
	{
		...
		case STV090x_SEARCH_DVBS2:
		{
			...
			if ((state->dev_ver <= 0x20) || (state->device == STX7111) || (state->device == STXH205))
			{
...
err:
	dprintk(1, "stv090x_delivery_search: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_start_search: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_get_agc2_min_level: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_srate_srch_coarse: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_srate_srch_fine: I/O error\n");
	return -1;
}
...
static int stv090x_get_dmdlock(struct stv090x_state *state, s32 timeout)
{
	...
	while ((timer < timeout) && (!lock))
	{
		...
		{
			dprintk(1,"Demodulator acquired LOCK\n");
		}
		timer += 4;
	}
	if (lock)
...
err:
	dprintk(1, "stv090x_chk_tmg: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_get_coldlock: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_search_car_loop: I/O error\n");
	return -1;
}
...
err:
	dprintk(1, "stv090x_sw_algo: I/O error\n");
	return -1;
}
...
static u8 stv090x_optimize_carloop(struct stv090x_state *state, enum stv090x_modcod modcod, s32 pilots)
{
	...
	if ((state->dev_ver == 0x20) && ((state->device == STX7111) || (state->device == STXH205)))
	{
		...
		printk("%s STXH205 cut 0x20 handling (modcod %d, pilots %d)\n", __func__, modcod, pilots);
		car_loop          = stxh205_s2_crl_cut20;
#else
...
static int stv090x_optimize_track(struct stv090x_state *state)
{
	...
	switch (state->delsys)
	{
		...
		case STV090x_DSS:
		{
			...
			if ((state->dev_ver >= 0x30) && (state->device != STX7111) && (state->device != STXH205))
			{
				if (stv090x_get_viterbi(state) < 0)
				{
					goto err;
			...
			else if ((state->dev_ver >= 0x20) && ((state->device == STX7111) || (state->device == STXH205)))
			{
				...
				else //if ((srate>=7000000) && (srate < 15000000))
				{
					if (STV090x_WRITE_DEMOD(state, ACLC, 0x0c) < 0)
					{
						goto err;
...
static enum stv090x_signal_state stv090x_algo(struct stv090x_state *state)
{
	...
	else
	{
		...
		if (state->dev_ver >= 0x20)
		{
			...
			else if ((state->algo == STV090x_WARM_SEARCH) && (state->device != STX7111) && (state->device != STXH205))
			{
...
err:
	dprintk(1, "stv090x_algo: I/O error\n");
	return -1;
}
...
static int stv090x_setup(struct dvb_frontend *fe)
{
	...
	else if (state->device == STXH205)
	{
		dprintk(10, "Initializing STXH205\n");
		stv090x_initval = stxh205_initval;
		t1_size = ARRAY_SIZE(stxh205_initval);
		t2_size = 0;
	}
	/* STV090x init */
	/* Stop Demod */
