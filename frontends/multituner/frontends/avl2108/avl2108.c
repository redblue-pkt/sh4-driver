...
static int avl2108_sleep(struct dvb_frontend *fe)
{
	...
	if (ret != AVL2108_OK)
	{
		dprintk(1, "%s Tuner cpu halt failed!\n", __func__);
		return -1;
	}
	return 0;
}
...
static int avl2108_set_frontend(struct dvb_frontend *fe, struct dvb_frontend_parameters *p)
{
	...
	if (cnt == 0)
	{
		dprintk(1, "%s: Tuner cpu halt failed ... resetting demod\n", __func__);
	...
	dprintk(10, "%s: Tuner successfully set!\n", __func__);
...
static void avl2108_register_frontend(struct dvb_adapter *dvb_adap, struct socket_s *socket)
{
	...
	if (frontend == NULL)
	{
		dprintk(1, "%s: avl2108 attach failed\n", __func__);
...
static int avl2108_demod_detect(struct socket_s *socket, struct frontend_s *frontend)
{
	...
	if (pin != NULL)
	{
		...
		else
		{
			...
			if (ret != AVL2108_OK)
			{
				dprintk(1, "%s: Failed to detect avl2108 0x%x\n", __func__, b1[0]);
				stpio_free_pin(pin);
				return -1;
			}
			dprintk(20, "%s: Detected avl2108\n", __func__);
		}
	}
	else
	{
...
static int avl2108_probe(struct platform_device *pdev)
{
	...
	dprintk(10, "%s: Found frontend \"%s\" in platform config\n", frontend_cfg->name);
